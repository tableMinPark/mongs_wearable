...
class MongRepositoryImpl @Inject constructor(
    ...
    private lateinit var mongModel: LiveData<MongModel>

    override fun initSetMong() {
        CoroutineScope(Dispatchers.IO).launch {
            val firstMong = appDatabase.mongDao().findFirstMong()
            setMong(firstMong)
        }
    }
    override fun generateMong() {
        CoroutineScope(Dispatchers.IO).launch {
            // TODO : 몽 생성 (API)
            val newMong = Mong.of(
                MongResModel(
            ...
            appDatabase.mongDao().registerMong(newMong)

            val firstMong = appDatabase.mongDao().findFirstMong()
            setMong(firstMong)
        }
    }
    override fun setSlot(slotId: Long) {
        CoroutineScope(Dispatchers.IO).launch {
            val selectMong = appDatabase.mongDao().findBySlotId(slotId = slotId)
            setMong(selectMong)
        }
    }
    override fun getMong(): LiveData<MongModel> {
        return this@MongRepositoryImpl.mongModel
    }

    private fun setMong(mong: LiveData<Mong>) {
        this@MongRepositoryImpl.mongModel = mong.map {
            MongModel(
                slotId = it.slotId,
                mongId = it.mongId,
                born = it.born,
                ...
                nextMongCode = it.nextMongCode,
                stateCode = it.stateCode,
                nextStateCode = it.nextStateCode,
