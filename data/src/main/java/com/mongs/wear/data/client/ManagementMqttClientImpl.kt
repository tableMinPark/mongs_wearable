...
class ManagementMqttClientImpl @Inject constructor(
    ...
    private val managementMqttApi: ManagementMqttApi,
    ...
    override suspend fun setConnection() {
        try {
            if (mqttFlag == MqttCode.STOP) {
                managementMqttApi.connect(messageCallback = messageCallback)
        ...
        try {
            if (mqttFlag == MqttCode.PAUSE_STOP) {
                ...
                if (eventMemberTopic.isNotBlank()) {
                    resetMember()
                    managementMqttApi.subscribe(topic = eventMemberTopic)
                }
                if (eventMongTopic.isNotBlank()) {
                    resetSlot()
                    managementMqttApi.subscribe(topic = eventMongTopic)
                }
            }
        } catch (e: MqttException) {
            e.printStackTrace()
    ...
    override suspend fun pauseConnect() {
        try {
            if (mqttFlag == MqttCode.CONNECT) {
                ...
                if (eventMemberTopic.isNotBlank()) {
                    managementMqttApi.disSubscribe(topic = eventMemberTopic)
                }
                if (eventMongTopic.isNotBlank()) {
                    managementMqttApi.disSubscribe(topic = eventMongTopic)
                }
                managementMqttApi.disConnect()
            }
        } catch (e: MqttException) {
            throw ClientException(
