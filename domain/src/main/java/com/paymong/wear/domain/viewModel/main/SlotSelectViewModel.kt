...
class SlotSelectViewModel @Inject constructor(
    ...
    fun setSlot(slotId: Long) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            mongRepository.setSlotId(callback = { _processCode.postValue(SlotSelectCode.END) }, slotId = slotId)
        }
    }
    ...
    fun generateMong() {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            mongRepository.generateMong(callback = { _processCode.postValue(SlotSelectCode.END) })
        }
    }
}